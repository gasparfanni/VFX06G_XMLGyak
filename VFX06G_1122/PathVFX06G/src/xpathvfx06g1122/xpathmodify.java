...
public class xpathmodify {
	public static void main(String[] args) {
		try {
			File inputFile = new File("VFX06G_orarend.xml");
			DocumentBuilderFactory dbFactory = DocumentBuilderFactory.newInstance();
			DocumentBuilder dBuilder = dbFactory.newDocumentBuilder();
			Document doc = dBuilder.parse(inputFile);
			doc.getDocumentElement().normalize();

			modifyElementValue(doc, "5", "oktato", "Nagy Árpád");

			modifyElementValue(doc, "9", "oktato", "Vass Ádám");

			modifyElementValue(doc, "5", "targy", "Számvitel");
			...
			StreamResult result = new StreamResult(new File("VFX06G_orarend1.xml"));
